...
//  5.2.2IR01B0328 游昆定制版本

#import <Foundation/Foundation.h>

typedef void (^YKUniResultListener)(NSDictionary * _Nonnull data);

@interface YKUniAuthHelper : NSObject

+(YKUniAuthHelper *_Nonnull) getInstance;
 ...
 */
-(void) getAccessCodeWithAppID:(NSString* _Nonnull)appId appSecret:(NSString* _Nonnull) appSecret timeout:(double)timeout listener:(YKUniResultListener _Nonnull) listener;

/**
 获取号码认证用的accessCode，与getAccessCode接口的区别是：回调中不会带有脱敏手机号码。
 */
-(void) mobileAuthWithAppID:(NSString* _Nonnull)appId appSecret:(NSString* _Nonnull) appSecret timeout:(double)timeout listener:(YKUniResultListener _Nonnull) listener;
